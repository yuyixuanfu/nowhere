...
def _local_cjk_serif() -> Optional[dict]:
    """乌有乡补丁: 优先本机中文衬线(楷/宋/Noto),离线在,不用下载。"""
    import sys
    candidates = []
    ...
    env_font = os.environ.get("NOWHERE_POSTER_FONT")
    if env_font:
        candidates.insert(0, env_font)
    for path in candidates:
        if os.path.exists(path):
            return {"bold": path, "regular": path, "light": path}
    return None
